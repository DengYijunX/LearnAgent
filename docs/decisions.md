...
## Decision 005：第一阶段延后真实 Web Search 和 GitHub API
...
搜索和完整 GitHub 仓库分析很重要，但真实外部接入会带来服务商选择、凭据、限流、解析质量和 token 预算等问题。

决策：

第一阶段定义可替换的 tool 接口。Web Search 暂时使用 mock 实现；GitHub API 接入后置。公开 README 读取作为低风险真实能力，已在 Decision 013 中单独记录。

理由：

这样可以先验证系统架构，不会过早绑定搜索 API 或 GitHub 凭据，同时保留后续替换真实实现的入口。

影响：

Workflow 必须依赖 Tool 接口和 ToolRegistry，而不是具体 mock 类。真实 Web Search、GitHub API、私有仓库访问和更完整的仓库分析应在后续确认后再加入。
...
## Decision 012：第一阶段接入真实 ReadUrlTool
...
Web Search 和 GitHub API 仍不接入真实外部服务，但文档链接读取是 LearnAgent 学习闭环中的低风险真实能力。

决策：

默认工具池中的 `read_url` 使用真实 `ReadUrlTool`。该工具仅支持 `http` 和 `https` URL，使用标准库读取页面并提取可见文本。原 `MockReadUrlTool` 保留用于架构测试。

理由：

真实网页读取能让 LearnAgent 从 mock 资料过渡到真实资料处理，同时不引入大型依赖或付费搜索 API。

影响：

`scripts/smoke_read_url_real.py` 用于手动验证真实网页读取。真实搜索 API 和 GitHub API 仍需后续单独确认。

## Decision 013：公开 GitHub README 分析不使用 GitHub API
...
用户确认第一阶段不接 GitHub API，也不引入 GitHub token；但可以支持公开 README / URL 的基础读取能力，作为后续仓库分析的低风险入口。

决策：

默认工具池中的 `github_repo_analyzer` 使用 `GitHubRepoAnalyzerTool`。该工具只解析公开 GitHub 仓库 URL，并尝试读取 `raw.githubusercontent.com` 上 main/master 分支的 `README.md`。它不调用 GitHub API，不读取 GitHub token，也不获取私有仓库内容。

理由：

公开 README 分析可以让 LearnAgent 先具备“理解仓库学习价值”的最小真实能力，同时避免凭据、权限、限流和 API 依赖复杂度。工具层接口保持稳定，后续可以在确认后扩展到目录结构、依赖文件和 GitHub API adapter。

影响：

`scripts/smoke_github_repo_real.py` 用于手动验证公开 README 读取。真实 GitHub API、私有仓库访问、目录结构分析和依赖文件分析仍需后续单独确认。
